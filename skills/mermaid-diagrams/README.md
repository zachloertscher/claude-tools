...
The skill itself is [`SKILL.md`](SKILL.md); everything below is what it produces.
...
```bash
cp -R . ~/.claude/skills/mermaid-diagrams/
```

Or into a single project's `.claude/skills/`. Claude Code picks it up with no restart.
